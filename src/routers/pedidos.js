...
function makePedidosRouter() {
    ...
    // pagar pedido
    router.post("/pedidos/:idPedido", midLogin, midMetodoPago, async (req, res) => {
        try {
        ...
        try {
            ...
            if (usuarioId === pedido.usuarioId) {
                if (pedido.estado === 1) {
                    ...
                    await pedido.save();
                    await session.commitTransaction();
                    res.status(200).json(`Producto ${producto.nombre} agregado`);
                } else {
                    await session.abortTransaction();
        ...
        try {
            ...
            if (usuarioId === pedido.usuarioId) {
                if (pedido.estado === 1) {
                    ...
                    await pedido.save();
                    await session.commitTransaction();
                    res.status(200).json(`Producto ${producto.nombre} eliminado del pedido`);
                } else {
                    await session.abortTransaction();
                    res.status(200).json(`El pedido ${pedido.id} ya ha sido abonado y no puede modificarse`)
                }
    ...
    //modificar pedido
    router.put("/pedidos/:idPedido", authAdmin, midLogin, midIdPedido, midModificarPedido, async (req, res) => {
        try {
            const idPedido = Number(req.params.idPedido);
